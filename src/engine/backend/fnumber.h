...
public:
	...
	// LP64 portability: `long`/`unsigned long` are 64-bit and a distinct type
	// from (u)int64_t on macOS/Linux, so a bare `long` argument is ambiguous
	// without these. On LLP64 Windows they are distinct 32-bit overloads. Both
	// delegate to the 64-bit integer paths (value-preserving on every target).
